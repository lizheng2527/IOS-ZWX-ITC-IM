...
//    self.sign = [[NIMSDK sharedSDK].userManager userInfo:userId].userInfo.sign;
    self.sign = [[NSUserDefaults standardUserDefaults]valueForKey:USER_DEFAULT_SIGNATURE];
...
- (void)onDone:(id)sender{
    ...
    [handler changeUserInfoWithSex:nil BirthDay:nil Email:nil MobiePhone:nil Signature:self.sign andStatus:^(BOOL successful) {
        [SVProgressHUD dismiss];
        [[NSUserDefaults standardUserDefaults]setValue:self.sign forKey:USER_DEFAULT_SIGNATURE];
         [[NSUserDefaults standardUserDefaults]synchronize];
         
         UINavigationController *nav = wself.navigationController;
         [nav.view makeToast:@"签名设置成功"
                    duration:2
