...
@interface AssetCheckController ()<UIScrollViewDelegate>
...
@implementation AssetCheckController
...
- (NSMutableArray *)dataArray {
    ...
    if (_dataArray == nil) {
        ...
        NSArray * array = [NSArray arrayWithObjects:@{@"title":@"全部",@"status":@"0"},@{@"title":@"待审核",@"status":@"0"},@{@"title":@"已通过",@"status":@"0"},@{@"title":@"未通过",@"status":@"0"}, nil];
        self.dataArray = [TitleModel objectArrayWithKeyValuesArray:array];
        NSLog(@"123");
    }
    return _dataArray;
}
...
- (void)setupChildVc {
    for (int i = 0; i <= _indexAll; i ++) {
        AssetCheckDiliverController * ContentView = [[AssetCheckDiliverController alloc] init];
