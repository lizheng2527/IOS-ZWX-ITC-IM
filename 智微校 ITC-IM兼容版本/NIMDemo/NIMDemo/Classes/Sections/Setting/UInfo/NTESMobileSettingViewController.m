...
//    self.mobile = [[NIMSDK sharedSDK].userManager userInfo:userId].userInfo.mobile;
    self.mobile = [[NSUserDefaults standardUserDefaults]valueForKey:USER_DEFAULT_MOBIENUM];
...
- (void)onDone:(id)sender{
    ...
    else if(![NSString isMobileNumber:self.mobile])
    {
    ...
    [handler changeUserInfoWithSex:nil BirthDay:nil Email:nil MobiePhone:self.mobile Signature:nil andStatus:^(BOOL successful) {
        [SVProgressHUD dismiss];
        [[NSUserDefaults standardUserDefaults]setValue:self.mobile forKey:USER_DEFAULT_MOBIENUM];
        [[NSUserDefaults standardUserDefaults]synchronize];
        
        UINavigationController *nav = wself.navigationController;
        [nav.view makeToast:@"手机设置成功"
                   duration:2
                   position:CSToastPositionCenter];
        [nav popViewControllerAnimated:YES];
        
    } failure:^(NSError *error) {
        [wself.view makeToast:@"手机设置失败，请重试"
                     duration:2
                     position:CSToastPositionCenter];
        [SVProgressHUD dismiss];
    }];
