...
+(NSMutableArray *)LoadingMessagesStartWithChatArray:(NSMutableArray *)chatArray SessionId:(NSString *)sessionId
{
    NSMutableArray *messages = [NSMutableArray new];
    
    [chatArray enumerateObjectsUsingBlock:^(SSChatMessageModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSMutableDictionary *dic = [NSMutableDictionary dictionary];
        [dic setValue:obj.content forKey:@"text"];
        [dic setValue:@"1" forKey:@"type"];
        [dic setValue:obj.sendTime forKey:@"date"];
        [dic setValue:obj.id forKey:@"messageId"];
        [dic setValue:[obj.kind isEqualToString:@"0"] ? @"2":@"1" forKey:@"from"];
        [dic setValue:[NSString stringWithFormat:@"%@%@",BaseURL,obj.photoUrl] forKey:@"headerImg"];
        [dic setValue:sessionId.length?sessionId:@"123" forKey:@"sessionId"];
...
+(NSMutableArray *)LoadingMessagesStartWithChat:(NSString *)sessionId{
    ...
    NSDictionary *dic2 = @{@"text":@"您好,随时给我打电话13533334444",
...
+(SSChatMessagelLayout *)getMessageWithDic:(NSDictionary *)dic{
    ...
    NSLog(@"headerImg=====   %@",dic[@"headerImg"]);
    
    //判断时间是否展示
    message.messageTime = [NSTimer getChatTimeStr:[NSTimer getStampWithTime:dic[@"date"]]];
...
+(void)sendMessage:(NSDictionary *)dict sessionId:(NSString *)sessionId messageType:(SSChatMessageType)messageType messageBlock:(MessageBlock)messageBlock{
    ...
    NSString *headUrlString = [NSString stringWithFormat:@"%@%@",BaseURL,[[NSUserDefaults standardUserDefaults] valueForKey:USER_DEFAULT_HeadPortraitUrl]];
    ...
    switch (messageType) {
        case SSChatMessageTypeText:{
            ...
            [messageDic setValue:headUrlString forKey:@"headerImg"];
        }
            break;
