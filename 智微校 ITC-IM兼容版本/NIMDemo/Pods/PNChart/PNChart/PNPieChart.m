...
- (void)strokeChart{
    ...
    for (int i = 0; i < _items.count; i++) {
        ...
        if (![descriptionLabel.text hasPrefix:@"0%"] ) {
            
            PNPieChartDataItem *dataModel = _items[i];
            NSArray *array = [descriptionLabel.text componentsSeparatedByString:@"\n"];
            if (array.count >=2) {
                descriptionLabel.text = [NSString stringWithFormat:@"%@(%.0lf次)\n%@",array[0],dataModel.value,array[1]];
            }
