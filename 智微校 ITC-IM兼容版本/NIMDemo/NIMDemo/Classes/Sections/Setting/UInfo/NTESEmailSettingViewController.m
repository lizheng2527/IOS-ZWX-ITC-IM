...
//    self.email = [[NIMSDK sharedSDK].userManager userInfo:userId].userInfo.email;
    self.email = [[NSUserDefaults standardUserDefaults]valueForKey:USER_DEFAULT_EMAIL];
...
- (void)onDone:(id)sender{
    ...
    else if(![NSString isValidEmail:self.email])
    {
        [self.view makeToast:@"邮箱格式错误,请检查" duration:2.0 position:CSToastPositionCenter];
        return;
    }
    ...
    [handler changeUserInfoWithSex:nil BirthDay:nil Email:self.email MobiePhone:nil Signature:nil andStatus:^(BOOL successful) {
        [SVProgressHUD dismiss];
        [[NSUserDefaults standardUserDefaults]setValue:self.email forKey:USER_DEFAULT_EMAIL];
        [[NSUserDefaults standardUserDefaults]synchronize];
        
        UINavigationController *nav = wself.navigationController;
        [nav.view makeToast:@"邮箱设置成功"
                   duration:2
