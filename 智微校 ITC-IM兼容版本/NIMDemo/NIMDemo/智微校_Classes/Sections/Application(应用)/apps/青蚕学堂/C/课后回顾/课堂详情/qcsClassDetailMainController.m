//
//  qcsClassDetailMainController.m
//  NIM
//
...
@interface qcsClassDetailMainController ()<UIScrollViewDelegate>
...
@implementation qcsClassDetailMainController
{
...
- (NSMutableArray *)dataArray {
    ...
    if (_dataArray == nil) {
        ...
        NSArray * array = [NSArray arrayWithObjects:@{@"title":@"手写笔记",@"status":@"0"},@{@"title":@"板书记录",@"status":@"0"},@{@"title":@"选择题",@"status":@"0"},@{@"title":@"教学课件",@"status":@"0"},nil];
...
- (void)setupChildVc {
    for (int i = 0; i <= _indexAll; i ++) {
        qcsClassDetailInsideController * ContentView = [[qcsClassDetailInsideController alloc] init];
        ...
        ContentView.userType = self.userType;
        
        if ([self.userType isEqualToString:@"student"]) {
            ContentView.studentXZTDatasource = [NSMutableArray arrayWithArray:_studentXZTDatasource];
            ContentView.studentSXBJDatasource = [NSMutableArray arrayWithArray:_studentSXBJDatasource];
        }
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    ...
    willShowVc.view.backgroundColor = [UIColor QCSBackgroundColor];
