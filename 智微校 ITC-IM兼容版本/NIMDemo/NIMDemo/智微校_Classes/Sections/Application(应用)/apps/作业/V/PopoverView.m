...
-(id)initWithPoint:(CGPoint)point titles:(NSArray *)titles images:(NSArray *)images
{
    ...
    if (self) {
        ...
        indexArray = [NSMutableArray arrayWithArray:images];
...
-(CGRect)getViewFrame
{
    ...
    frame.size.height = [self.titleArray count] * ROW_HEIGHT + SPACE + kArrowHeight + 44;
    
    
    for (HWCourseModel  *titleModel  in self.titleArray) {
        CGFloat width =  [titleModel.courseName sizeWithFont:TITLE_FONT constrainedToSize:CGSizeMake(300, 100) lineBreakMode:NSLineBreakByCharWrapping].width;
...
//    frame.size.width = [UIScreen mainScreen].bounds.size.width - 80;
    if ([self.titleArray count] == [self.imageArray count]) {
        frame.size.width = 10 + 25 + 10 + frame.size.width + 160;
    }else{
        frame.size.width = 10 + frame.size.width + 160;
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HWCourseModel *model = _titleArray[indexPath.row];
...
//        cell.frame = tableView.frame;
    ...
    NSInteger tempIndex = [indexArray[0] integerValue];
...
//    if ([model.courseName isEqualToString:[self.titleArray[_index] courseName]]) {
//        cell.imageview.hidden = NO;
//    } else {
//        cell.imageview.hidden = YES;
//    }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    UIView* customView = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, self.frame.size.width, 44.0)];
    customView.backgroundColor = [UIColor whiteColor];
    UILabel * headerLabel = [[UILabel alloc] initWithFrame:CGRectZero];
    headerLabel.backgroundColor = [UIColor clearColor];
    headerLabel.textColor = RGB(54, 191, 181);
    headerLabel.font = [UIFont boldSystemFontOfSize:16];
    headerLabel.frame = CGRectMake(10.0, 0.0, 300.0, 44.0);
    headerLabel.text = @"选择科目";
