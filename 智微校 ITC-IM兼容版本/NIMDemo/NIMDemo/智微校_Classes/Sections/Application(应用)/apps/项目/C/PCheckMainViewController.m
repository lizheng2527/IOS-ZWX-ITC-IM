...
@interface PCheckMainViewController ()<UIScrollViewDelegate>
...
@implementation PCheckMainViewController
...
- (NSMutableArray *)dataArray {
    ...
    if (_dataArray == nil) {
        ...
        NSArray * array = [NSArray arrayWithObjects:@{@"title":@"全部",@"status":@"0"},@{@"title":@"未审核",@"status":@"0"},@{@"title":@"已通过",@"status":@"0"}, nil];
...
- (void)setupChildVc {
    for (int i = 0; i <= _indexAll; i ++) {
        PCheckDetailController * ContentView = [[PCheckDetailController alloc] init];
...
-(void)addItemAction:(id)sender
{
    ...
    UIAlertAction* addVisitRecord = [UIAlertAction actionWithTitle:@"添加拜访记录" style:UIAlertActionStyleDefault                                                                 handler:^(UIAlertAction * action) {
        
    }];
    UIAlertAction* addServerApplication = [UIAlertAction actionWithTitle:@"添加服务申请" style:UIAlertActionStyleDefault                                                                 handler:^(UIAlertAction * action) {
        
    }];
    UIAlertAction* addServerRecord = [UIAlertAction actionWithTitle:@"添加服务记录" style:UIAlertActionStyleDefault                                                                 handler:^(UIAlertAction * action) {
