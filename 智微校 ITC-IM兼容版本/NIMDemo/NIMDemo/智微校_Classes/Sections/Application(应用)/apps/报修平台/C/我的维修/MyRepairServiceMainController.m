//
//  MyRepairServiceMainController.m
//  NIM
//
...
@interface MyRepairServiceMainController ()<UIScrollViewDelegate>
...
@implementation MyRepairServiceMainController
...
- (NSMutableArray *)dataArray {
    ...
    if (_dataArray == nil) {
        ...
        NSArray * array = [NSArray arrayWithObjects:@{@"title":@"全部",@"status":@"0"},@{@"title":@"维修中",@"status":@"0"},@{@"title":@"已完成",@"status":@"0"},nil];
...
- (void)setupChildVc {
    for (int i = 0; i <= _indexAll; i ++) {
        MyRepairServiceDetailController * ContentView = [[MyRepairServiceDetailController alloc] init];
