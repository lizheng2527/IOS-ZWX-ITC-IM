...
@interface MRDMainController ()<UIScrollViewDelegate>
...
@implementation MRDMainController
- (NSMutableArray *)dataArray {
    
    if (_dataArray == nil) {
        
        self.dataArray = [[NSMutableArray alloc] init];
        NSArray * array = [NSArray arrayWithObjects:@{@"title":@"报修单详情",@"status":@"0"},@{@"title":@"维修过程记录",@"status":@"0"},nil];
...
- (void)setupChildVc {
    for (int i = 0; i <= _indexAll; i ++) {
        MRDetailController * ContentView = [[MRDetailController alloc] init];
        ContentView.viewTag = 1001 + i;
        TitleModel * model = self.dataArray[i];
        ContentView.title = model.title;
        ContentView.repairID = self.repairID;
