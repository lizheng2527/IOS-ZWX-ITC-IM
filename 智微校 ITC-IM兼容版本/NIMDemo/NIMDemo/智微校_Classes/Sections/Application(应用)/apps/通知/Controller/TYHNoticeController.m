...
@interface TYHNoticeController ()<UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate,TYHMessageHeaderViewDelegate>
...
- (void)editCell{
    ...
    if (isEditing) {
        ...
        if (self.allMessages != nil && ![self.allMessages isKindOfClass:[NSNull class]] && self.allMessages.count != 0) {
            ...
            self.deleteBtn.frame = CGRectMake(self.deleteBtn.frame.origin.x, [UIScreen mainScreen].bounds.size.height  - self.deleteBtn.frame.size.height - self.tabBarController.tabBar.frame.size.height , self.deleteBtn.frame.size.width, self.deleteBtn.frame.size.height);
            [self creatLeftItem];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (section == 0) {
        ...
        NSString *headerContent = [[NSUserDefaults standardUserDefaults] valueForKey:headerContentKey];
        NSString *headerTime = [[NSUserDefaults standardUserDefaults] valueForKey:headerTimeKey];
        ...
        _headerView.nameLabel.text = @"留言中心";
        _headerView.messageLabel.text = @"暂无最新留言消息";
        [_headerView.badgeView HIDDEN:YES];
        _headerView.delegate = self;
        
        if (headerTime.length && headerContent.length) {
            _headerView.timeLabel.text = headerTime;
            _headerView.nameLabel.text = @"您有新的留言";
            _headerView.messageLabel.text = headerContent;
...
{
    TYHMessageListController *listView = [TYHMessageListController new];
    
    NSString *userIDString = [[NSUserDefaults standardUserDefaults]valueForKey:USER_DEFAULT_USERID];
    NSString *headerContentKey = [NSString stringWithFormat:@"%@%@",NewV3PushMessageContent,userIDString];
    NSString *headerTimeKey = [NSString stringWithFormat:@"%@%@",NewV3PushMessageTime,userIDString];
    
    [[NSUserDefaults standardUserDefaults]setValue:@"" forKey:headerTimeKey];
    [[NSUserDefaults standardUserDefaults]setValue:@"" forKey:headerContentKey];
